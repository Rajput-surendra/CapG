...
void initState() {
  ...
  getBiddinglistApi() async {
    ...
    var request = http.MultipartRequest('POST', Uri.parse('$baseUrl/bid_product_get'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            getBid?.data == null ? Center(child: CircularProgressIndicator()) : getBid!.data!.length == 0 ? Text("No Product found"):Container(
                          ...
                          Row(
                            children: [
                              ...
                              Column(
                                ...
                                children: [
                                  Text("${getBid?.data?[index].name}",style: TextStyle(color: colors.blackTemp,fontWeight: FontWeight.bold),),
                                  Text("₹ ${getBid?.data?[index].price}"),
                                  Text("${getBid?.data?[index].message}"),
                                ],
                              )
                            ],
                          )
